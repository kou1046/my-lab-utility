...
class ImageDownloader:
    ...
    def set_imgs_with_request(
        self, url: str, imgs_name: str, inner: bool = False
    ) -> None:  # inner=True → サムネイルクリック先の画像urlを取り出す
        res = requests.get(url)
        bs = BeautifulSoup(res.text, "html.parser")
        if inner:
            img_urls = [
                img_tag.parent["href"]
                for img_tag in bs.select("img")
    ...
    def set_imgs_with_selenium(
        self,
        url: str,
        imgs_name: str,
        inner: bool = False,  # loading_css_selectorを設定すると指定したセレクタのdomがなくなるまで待機する（読み込み対策）
        ...
        if inner:
            img_urls = [
                attr(get_parent(img_tag), "href")
                for img_tag in select("img")
                if get_parent(img_tag).tag_name == "a"
            ]
        else:
    ...
    def view(self) -> None:
        def btn_cmd(e, folder_: NamedImageFolder) -> None:
            ...
            viewer = ScrollImageViewer(
                title_f, img_sequence=[img.img for img in folder_.imgs]
            )
            del_btn = tk.Button(
                title_f,
                text="消去",
                command=lambda: [
                    folder_.pop_img(viewer.index),
                    viewer.del_img(viewer.index),
                ],
            )
            del_btn.pack()
            viewer.pack()
    ...
    def _filter_imgs(self, img_urls: Sequence[str]) -> list[str]:
        ...
        filter_exts = {"png", "jpg"}
        return [
            url for url in img_urls if (url is not None) and (url[-3:] in filter_exts)
        ]
